...
| Interactive HTML attack-path report | B | Ready for local review artifacts and demos. | HTML escaping, graph construction, dense layout, attack path, architecture, evidence paths, finding views, and clearer labels have regression tests. | Needs browser-render smoke tests with screenshots and pixel checks before treating visual UX as release-gate critical. |
